...
describe("composeSubagentPrompt", () => {
  it("default = 用写死在代码里那段", () => {
    const out = composeSubagentPrompt({ def: base, docs: [] });
    expect(out).toBe(`${DEFAULT_PREAMBLE}\n\n正文`);
  });

  it("off = 一段前置词都不加", () => {
    const def = { ...base, preamble: { mode: "off" } as const };
    expect(composeSubagentPrompt({ def, docs: [] })).toBe("正文");
  });

  it("custom 覆盖那一段，不是追加", () => {
    const def = { ...base, preamble: { mode: "custom", text: "只输出 JSON" } as const };
    const out = composeSubagentPrompt({ def, docs: [] });
    expect(out).toBe("只输出 JSON\n\n正文");
    expect(out).not.toContain(DEFAULT_PREAMBLE);
  });
  ...
  it("正文超长也截断——三个输入拼进同一个字符串,只给文档设限等于没设", () => {
    // 前置词关掉,好让长度这条断言只量正文那一段
    const def = {
      ...base,
      preamble: { mode: "off" } as const,
      instructions: "x".repeat(CONTEXT_DOC_LIMIT + 10),
    };
    const out = composeSubagentPrompt({ def, docs: [] });
