...
// 三期）。在此之前，任一团队的任一成员可以：无限刷 @Agent 烧 key、
...
/** 四档闸门的取值（stop 是 #957 第三批加的第四档）。数字的依据都是"一个人手动操作能有多快"，不是压测出来的：
    - say：**每一条 say 帧都要付一次**（#968）——它是"有没有资格开口"的
      粗闸，收在 frameHandler 查会员资格之前，点没点名都要付。群里连着
      说话，一分钟 30 条已经是很热闹的聊天了（突发 30）
    - turn：点了名的 say 帧**在 say 桶之外额外**付的价钱——每只被点到的
      agent 都可能起一次真花钱的模型调用，数量由 say() 里 resolveTargets
      解出的真实 targets 决定，不是客户端自报的。一分钟 4 次 = 平均 15
      秒一轮，比人类跟 agent 来回的节奏还宽松；突发 10 接住"连着补几句
      上下文"这种真实形态
...
export type ThrottleKind = "say" | "turn" | "create" | "stop" | "files" | "wiki" | "call";
