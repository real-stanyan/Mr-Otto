...
/** 上游回 5xx / 429 是立刻回落，只有「挂住不回」才付满这一格。
    **这 20 秒是影子期的测量窗口，不是生产值**（ADR-0301 补记三 / #1300）。三步：
    ① 2026-09-21 真机量出来，派活真正跑的那条路（runtime 在 VPS 上，Cloudflare 的 HEL
       ...
       超时，调到 2000 / 3000 / 5000 一样是 100%。病根不在这个数，在 #1304：那边的打点
       （ADR-0305）量出 HEL 上**一趟 DO 往返就是 ~344ms**，而这条调用走 `hold` + `settle`
       两趟，于是**地板是 ~700ms 再加上游那一段**，与决策模型快不快无关 —— 1200 因此不是
       「偏紧」是**结构上不够**，而那 700ms 只有减少趟数或改放置才降得下来。
