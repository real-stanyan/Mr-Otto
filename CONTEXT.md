...
| 错误分类（errorClass） | 模型 API 错误的三类标记：`rate-limit` / `retryable`（瞬态）/ `fatal`，**抛错的地方分类**（状态码还在手上，不从错误文案倒推），贴属性不建子类。与 retryable 标记正交：class 是种类，retryable 是种类×流位置的重试许可（首 token 后的瞬态错不可重试）。turn_ended 可选 errorClass 字段补记分类，error 仍存原文 | ADR-0087 决定 1；`src/model/errorClass.ts` |
| 后台回注（background re-injection） | bash `run_in_background` 起的任务（world.execDetached，不绑 turn 信号、30 分钟超时）完成后，结果以**新 turn** 注回会话——不 mid-splice（改投影中段 = prefix cache 全废，ADR-0073）。turn 在跑就攒（pendingBg），正常收口后合并注回；aborted 不排空。`background_task_completed` 事件是 log-only 审计，模型可见载体是回注 turn 的 user_message | ADR-0087 决定 2；`src/main/backgroundTasks.ts` |
| 沙箱事实上报（ExecResult.sandbox） | 命令跑完了但沙箱在过程中拦了什么/自身出了什么状况——随 ExecResult 回来的**事实**（与 SandboxDeniedError 的"整条被拒抛错走升级环"互补）。denials（拦了=约束在工作）与 failures（坏了=约束可能失效）严格分开；判定责任在沙箱实现侧，不做 stderr 启发式。v1 无生产者，协议即测试 | ADR-0087 决定 3；`src/world/sandbox.ts`（SandboxEnforcementFacts） |
| 不变量校验（checkInvariants） | 事件流结构合法性的可执行断言（工具调用三方配对 / toolCallId 唯一 / turn 不双收口），resume 修复**之后**跑、只告警不拦——违例是写入方有 bug 的线索，不是拒读理由（拒读那道门只属于 assertReplayable）。合法反例（ts 乱序/fork 双创建/steer 多 user_message/修复尾巴）明文豁免 | ADR-0087 决定 4；`src/session/invariants.ts` |
| keyless 回放 | 「无 API key 也能查看历史会话与轨迹」的契约：key 门只在发送那一刻（modelRoute blocked），启动/恢复/投影全程不碰 key。功能今天就通，靠单元 + e2e 双层测试钉住不回退 | ADR-0087 决定 5；`tests/main/keylessReplay.test.ts`、`tests/e2e/keylessReplay.e2e.ts` |
