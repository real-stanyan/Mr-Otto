...
| MCP（Model Context Protocol） | 一个标准协议，让 Otto 作为 **client** 连外部**server**，把 server 提供的 tools / resources / prompts 接进来。v1.x 只做 client 这一半，不做 server；不算插件系统——server 是跨进程外部程序，不向 Otto 进程注入代码 | ADR-0048、ADR-0049 |
| MCP server | 一个外部程序，通过标准协议（stdio 或 streamable-http）向 client 提供 tools / resources / prompts。Otto 只认协议返回的形状（`McpClientConn`），不加载、不执行 server 的任何代码 | ADR-0048；配置见 `~/.otter/mcp.json` |
| transport（stdio / streamable-http） | MCP 连接的传输方式：`stdio` = 本地 `spawn` 一个子进程，走标准输入输出对话；`streamable-http` = 连一个远程 HTTP 端点。配置里有 `command` 走前者，有 `url` 走后者，两者都有则报错不猜 | ADR-0049；`src/main/mcpClient.ts` |
