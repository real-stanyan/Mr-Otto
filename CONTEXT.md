...
| 请求信封（request_envelope） | 每次模型调用前落盘的 log-only 审计快照：渲染后的 system 全文 + 工具声明表 + model/wireModel/thinking——请求里日志推不出的那半（工具表是磁盘/MCP 动态状态，thinking 是运行时偏好）。与上一条相同不落（典型会话一两条）；模型不可见；带 ignorable。用途：任何历史请求可从日志逐字节重构 | ADR-0085 决定 1、issue #383；engine.appendEnvelopeIfChanged |
| 单调守卫（ToolGuard） | Pre 钩子后、执行留痕前的 deny-only 闸：返回 reason = 拒绝（落 `tool_hook{action:"guard_deny"}`），返回 undefined = 弃权，**没有 allow**——注册顺序翻不了案。看到的是最终生效参数（审批改参/钩子改参之后），内置守卫在这复查 execpolicy forbidden。与钩子的分工：钩子 fail-open（超时弃权），守卫 fail-closed（受信代码不设超时） | ADR-0085 决定 2；`src/loop/middleware.ts`（ToolGuard） |
| 向前兼容拒读（ignorable） | 「旧版读新日志」的契约（硬规则只定义了反方向）：事件可标 `ignorable: true` = 不认识它的旧版本可安全跳过；resume 读到未知且未标的类型抛 UnknownSessionEventError（话术指向升级），不静默跳过。默认拒是刻意的：忘标的代价是多拒一次，静默跳过的代价是复活残缺会话。只把 resume 的门，列表/回看宽容 | ADR-0085 决定 3；events.assertReplayable |
| 合成收口（interrupted） | resume 时发现开着的 turn（最后一条 turn_ended 之后仍有 turn 活动）补 `turn_ended{outcome:"interrupted"}`——loop 永不产生这个值，「修的」和「跑出来的」永远可区分。追加不改写、幂等；崩溃空跑 turn 从此被 barrenTurns 正确跳出上下文 | ADR-0085 决定 4；agent.ts resume 修复块 |
| 新鲜区折叠（maxFreshToolOutputChars） | 投影对保真区 tool_result 的独立上限（默认 50K 字符；老区仍 400）——此前新鲜区无上限，read_file/MCP 一条超长输出直接吃穿窗口。日志存全文不动（UI/摘要人照读），折叠只住投影层；hermes 的 spill-to-disk 在本仓不需要：日志本身就是那块盘 | ADR-0085 决定 5；deriveMessages tool_result case |
