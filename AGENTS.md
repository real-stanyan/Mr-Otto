...
- `src/shared/sessionPackage.ts` 的 `PRIVACY_VERDICTS` — 分享会话的隐私闸是**每个事件类型都必须表态**的 `Record<type, "strip"|"keep">`，不是 denylist（#850）：新加事件类型不来这里写一笔，tsc 直接红。表态的问题只有一个——这条事件说的是「这段对话」还是「发送方这台机器 / 这个人的私事」，后者 strip。它是新事件类型检查清单的第七处（前六处：events.ts union + `KNOWN_EVENT_TYPES_MAP`、persistencePolicy、deriveMessages、deriveSections、threadGroups.isInvisible、deriveUsage）
