...
- `src/main/memoryFiles.ts` / `src/main/memorySync.ts` / `src/shared/memoryReconcile.ts` / `supabase/migrations/0019_memory_docs.sql` — 记忆跟账号走（ADR-0207，#852）：`memoryFiles` 是 accountConfig 下 `memories/` 的**唯一**读写口（架构断言：碰记忆路径符号的文件不许 import node:fs），写完回调汇进 `memorySync`（工具那条路经 `LocalWorld.onConfigWrite`）；对账是纯函数 `planReconcile`（后写胜、内容相同不动）；触发点只有本地写 + 登录恢复，不轮询；从云端写本地时 `muted`，否则死循环。项目档 key 含路径哈希实际不跨机——已知天花板
