...
- `src/shared/workspaceLock.ts` / `src/main/workspaceLockFile.ts` — 工作区互斥的**跨进程**那一半（两个 app 实例指同一个文件夹）。锁在机器级临时目录、按路径哈希取名——不进用户的仓库；陈旧锁靠心跳过期 + pid 探活自愈，**自愈比挡住更重要**（一次崩溃不该把文件夹永久锁死，ADR-0154）
