...
- `src/shared/workspaceExclusion.ts` — 同一个文件夹同一时刻只跑一条 turn（纯逻辑；接线在 `index.ts` 的 turn 起跑处，紧挨着 `runningSessions` 那条自检）。沙箱围的是路径，围不住共享的 `.git`；同家族（子会话/SideChat）不互斥（ADR-0151）
