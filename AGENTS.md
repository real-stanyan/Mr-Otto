...
- `src/shared/toolLoopGuard.ts` / `src/loop/engine.ts` 的 `loopFingerprints` — 退化循环护栏（ADR-0210，#891）：判据是**周期重复**不是「连续相同」——真机那条轨迹里相邻两圈从来不相等（同一组 14 条命令循环了 14 遍、190 次调用、两小时零进展、零信号），只认 `last === prev` 的护栏会全程沉默。命中**注一条话不停 turn**（ADR-0006 的无步数天花板原样成立），载体沿用 `user_message` + `origin`（`"loop_guard"`，同 ADR-0205 那条注入路径，所以不新增事件类型）。话注在这一圈 `tool_result` 全部落盘之后——插在 `assistant(tool_calls)` 与答复之间投影出来就是「有 tool_call 没答复」（ADR-0005 的教训）。喊完清空历史，判据作用域是一个 turn
