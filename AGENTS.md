...
- `src/shared/sessionWorktree.ts` / `src/main/sessionWorktreeService.ts` — 同一个项目上的**第二只**水獭拿一份独立工作副本（git worktree，落在 userData 不进用户的仓库）。第一只不隔离（用户点的文件夹就是他看得见的那个）；判据是「同一个仓库」不是「同一个路径」；水獭自己从系统提示里知道它在副本上、合回去要先问一句。推翻了 ADR-0152 对候选 1 的否决，那条互斥留作兜底（ADR-0156）
