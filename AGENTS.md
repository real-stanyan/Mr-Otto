...
- `tests/e2e/` — Playwright-electron 端到端（`npm run e2e`，不在 gate 里；GUI 改动的 PR 贴它的结果，ADR-0058）。既是冒烟，也是四张真机验收清单的落地处：`harness.ts` 换 `HOME` 做隔离、`fakeModel.ts` 是本机假模型，见 ADR-0075
