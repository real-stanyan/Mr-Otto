...
| `0036_task_sessions.sql` | 任务会话的云端日志：两张表（`task_sessions` / `task_session_events`，own-row RLS、写只走 `security definer` RPC 的 seq CAS）+ 六个 RPC（`task_append` / `task_pen_acquire` / `task_pen_release` 各带一个 `_as` 包装给 service_role）+ Storage bucket `task-attachments`（own-folder 四条策略） | ADR-0291，#1223。**未在 Cloud 上执行**（合并前由 controller 手动跑一次，跑完把这一格改成「已在 Cloud 上执行」+ 日期）。**前提：`supabase_realtime` publication 必须已存在**——0036 里那段 `alter publication … add table` 的异常块只吞 `duplicate_object`，publication 不存在时抛的是 `undefined_object`，整份 migration 会停在那儿。不跑的话桌面状态行写 off「云端还没有任务会话表（migration 0036 未执行）」并**不重试**（`missing_schema` 那条分支），任务会话照常只在本机；其余功能不受影响 |
