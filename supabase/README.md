...
| `0034_workspace_wiki_journal.sql` | 团队 wiki 的 journal 备份表（`workspace_wiki_journal`，追加式，`content` null = 删）：`/work/wiki/` 里的文件是事实，这张表单向备份 + 历史，恢复只发生在 `wiki/` 不存在时 | ADR-0282，#1140。不跑的话 runtime 每次写入 warn 一行、恢复路不可用，其余功能不受影响。**已在 Cloud 上执行**（2026-09-09，controller 手动跑的）——文件编号从 0033 改到 0034 是合并前的撞号改号（origin/main 先落了 `0033_model_route_tts.sql`，project ADR-0074），跑的是同一张表定义，表名才是认的凭据，文件编号不是 |
| `0035_cloud_session_participants.sql` | `workspace_sessions` 加两列（`participants` jsonb / `participants_window` bigint）：云会话侧栏「最近谁说过话」的头像投影 | ADR-0283，#1213。**已在 Cloud 上执行**（2026-09-10，controller 手动跑的：`supabase db query --linked -f`，跑前确认两列不存在、跑后核过 `information_schema.columns` 里 `participants` jsonb NOT NULL default `'[]'` / `participants_window` bigint NOT NULL default 0，并原样重跑一次验幂等守卫）。依赖它的只有这两个新列——`title` 早就存在，不受影响。读、写两侧各自容错：`src/main/supabaseWorkspacesApi.ts` 的 `fetchCloudParticipants` 把这两列拆成单独一条查询，列不存在时只让这个团队的 `participantUids` 整批回 `[]`（不拖累标题/归档等其余字段，也不连累整份云会话列表——早先的实现一度把它拼进主 select，那样 0035 落地前会让整个团队的云会话读不出来）；`services/runtime/src/cloudSessionMeta.ts` 的 `setParticipants` 拿到 42703 只记一行日志、不抛。文件编号原为 0034，合并 origin/main 时与另一条 lane（#1140）的 `workspace_wiki_journal` 撞号，那份先落地，这份改号（project ADR-0074，`tests/docs/migrationNumbers.test.ts`） |
| `0036_task_sessions.sql` | 任务会话的云端日志：两张表（`task_sessions` / `task_session_events`，own-row RLS、写只走 `security definer` RPC 的 seq CAS）+ 六个 RPC（`task_append` / `task_pen_acquire` / `task_pen_release` 各带一个 `_as` 包装给 service_role）+ Storage bucket `task-attachments`（own-folder 四条策略） | ADR-0291，#1223。**已在 Cloud 上执行**（2026-09-10，经维护者确认后 controller 手动跑的：`supabase db query --linked -f`，整份一次过——先 `supabase link` 拿 IPv4 pooler，直连 5432 走 IPv6 在本机不通；跑前核过 publication 在、两张表 / 九个函数 / bucket 都不在，跑后核过五条：表 + RLS + 0 行 / 策略只有 select 与 task_sessions 的 delete / publication + replica identity full / 九个函数的 ACL 与文件逐字一致 / 私有 bucket + 四条 storage 策略；之后原样重跑一次验幂等守卫，无报错、计数不变）。**前提：`supabase_realtime` publication 必须已存在**——0036 里那段 `alter publication … add table` 的异常块只吞 `duplicate_object`，publication 不存在时抛的是 `undefined_object`，整份 migration 会停在那儿。不跑的话桌面状态行写 off「云端还没有任务会话表（migration 0036 未执行）」并**不重试**（`missing_schema` 那条分支），任务会话照常只在本机；其余功能不受影响 |
| `0037_home_workspace_and_chats.sql` | 个人主场与聊天：`workspaces.kind`（team/home）+ 每人至多一个主场的唯一索引 + kind 不可变触发器 + 主场不收别人（换 `wsm_insert_owner`）+ 种子管理员换一句职责；`workspace_sessions` 加 `chat_kind` / `agent_ids` 两列 + 形状 CHECK + 每只智能体至多一条私聊的唯一索引 | ADR-0297，#1280。**已在 Cloud 上执行**（2026-09-20，经维护者明说后跑的：Management API `/database/query`，**按语句切开逐条发**——那个端点一次只回最后一条的结果，整份贴进去看不见前面几条是成功还是失败，切分器要尊重 `$$…$$`，否则 `do $$ … $$` 会被里面的分号切碎；12 条全过）。跑前核过八样（三列 / 两条 CHECK / 两个唯一索引 / 触发器）一个都不在，跑后逐个核过定义与文件一致、存量落进 `kind='team' × 2` 与 `chat_kind=(null) × 5`（两条 CHECK 的第一支），之后原样重跑一次验幂等守卫，12 条全 OK、七项计数一个没变。**与客户端发版的先后：先这份 → 再部署 runtime → 再发桌面**（daemon 启动那条 select 读这两列；cs 协议 19→20，旧桌面连不上新 runtime）。不跑的话客户端读这三列走单独一条容错查询，团队照常能用，只是第三栏建不出个人主场 |
