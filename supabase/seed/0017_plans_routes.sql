...
values
  ...
  --   ¥2 / ¥0.04 / ¥8 —— 输出上按四分之一的价扣用户额度，差额自己贴（#1241）。
  --   **时间轴要说准**：V4.1 Flash 的价 2026-09-10 04:00 UTC 才生效，那之前这一行是对的
  --   （195 条 usage_event 全在 9/04~9/09，之后零条）。所以这是刚发生、还没造成损失的错，
  --   不是陈年欠账 —— 「表现在错」与「表一直错」是两个断言，后者要查 usage_event 的时间戳
  ...
  -- DeepSeek V4 Pro（2026-09-10 同一页高峰档：$1.32 / $0.044 / $3.96）。
  --   **不是它涨价了**：官方邮件明写「V4 Pro 的价在服务期内不变」，而这一行原先记的
  --   ¥3.00 / ¥0.025 / ¥6.00 与 modelPricing.ts 自 2026-08-30 起写着的 $1.32 / $3.96 差 3.2 倍，
  --   定价页两次都站在后者那边 —— 是 2026-09-02 这次 seed 抄错了，错了八天没症状，
  --   因为这一行的 usage_event 是 0 条（它既不是最便宜那款也不是 models.at(-1)，Auto 与默认款都够不到）
