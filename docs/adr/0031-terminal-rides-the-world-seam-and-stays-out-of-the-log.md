...
> 这个号曾经有两份（issue #230）。另一份「thinking 是型号的属性」已改号为
> ADR-0072。2026-08-23 之前的 commit message 里写着 ADR-0031 的，两篇都有可能。
...
每终端保留末尾约 20 万字符（UTF-16 码元，即 JS 字符串 `.length` 的单位，不是编码后的
字节数——量字节需要在热路径上加一次 `TextEncoder`，不值这个成本）的环形缓冲。面板一关，
渲染层的 xterm 实例就没了，而 pty 还在吐输出（这是「关面板不杀进程」的产品前提）——
必须有人接住。放渲染层等于没放。
