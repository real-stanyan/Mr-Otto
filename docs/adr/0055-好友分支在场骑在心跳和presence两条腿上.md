...
- 隐私开关:先默认对好友公开;要关再开 issue。
  ...
  > **订正(2026-08-23,issue #236)**:「默认对**好友**公开」这句话在真库上不成立。
  > `profiles` 的 select policy 是 `profiles_select_authenticated`,`using` 就是 `true`
  > —— 任何拿到 authenticated JWT 的人都能 select 全表任意一行,不止好友。这条 policy
  > 是 0001 就有的、且是**故意的**(按邮箱精确搜好友依赖它,`checks/0001` 里有对应断言),
  > 0008 只是把两个新列放进了它的覆盖范围。所以真实的可见性是:
  > `repo_key` 是 hash(只泄露"同不同仓库",可接受),**`repo_branch` 是明文,对所有
  > 注册用户可见**;而注册是敞开的(#122),等于对任何愿意注册的人可见。
  >
  > 维护者 2026-08-23 拍板:**接受现状,不改**。#236 记着三条收法(列级 grant / 换 policy
  > 为「自己+好友」/ 拆表 `workspace_presence`)和各自的代价,已关;要改从那条 issue 起。
  > 因此本节这一行的准确说法是:**默认对所有注册用户公开**,而不是对好友公开。
...
- 如果 #77 修好且 Realtime 被证明长期稳定,心跳腿可以退化成纯在线点(去掉两列),但 ADR-0027 的理由(自托管链路不可靠)不变,大概率不会。
  ...
  > **进展(2026-08-23)**:#77 已修并关闭(根因是 Envoy 的 realtime 集群写死了一个
  > 删掉 `container_name` 之后就不存在的主机名,不是端口配错;公网 wss 握手已拿到 101)。
  > 但这只满足了前提的前半句 —— 「Realtime 被证明长期稳定」还没有证据,而这次的根因
  > 恰恰是"链路可以因为一个无关的部署决定静默死掉、死了半个多月没人发现",正是 ADR-0027
  > 说自托管链路不可靠的那类事。所以两条腿保留,两列不去掉。
