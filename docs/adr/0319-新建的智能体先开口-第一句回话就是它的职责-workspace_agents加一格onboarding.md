...
- 每建一条**新**私聊（桌面草稿发第一句建的也算）多一条条件 UPDATE（抢那一格），排在 `created` 回执之前；0041 没跑之前每条新私聊还会记一行抢不到的日志。
- 落行那一步的回执丢了（行其实落了）：再点一次会报「已有同名的智能体」；那一行刷新后照样出现在名册上，点进去能用，最坏是之后从草稿发第一句时双答一次。
