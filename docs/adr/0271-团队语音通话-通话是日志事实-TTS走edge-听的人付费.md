...
- 用户语音输入（STT）**由 ADR-0273 补上**（macOS 原生本机识别，常开麦、半双工）；真人之间语音、手机端：不做。
- **要 `wrangler secret put MINIMAX_API_KEY` + 部署 edge + 跑 0033 + 部署 runtime（协议 17
  ...
  kind 当 chat，先插行会让 TTS 那款漏进对话选单、还是 Auto 的 hard 档）；2026-09-09 真机冒烟已过（#1163 评论）。
- **拉进通话之后没人先开口**——真机第一次用就撞上（「开始通话 → 等 → 沉默」），由 ADR-0272 补：新增成员各起一轮带 `greeting` 记号的招呼。
