...
- 状态：已采纳
- 日期：2026-09-08
- 关联：#1107；ADR-0199（云会话）、ADR-0250（云会话只剩输入指示器）、ADR-0214（流式光标）、#278（本机 delta 合帧）、#341（delta 不落日志的契约）；协议 15→16（开分支时是 13→14，合并时 #1102/#1103 已各占一个号，按纪律后合的再进一）
...
- 已知代价：中继多一路扇出（50ms × 在线成员 × 同时在写的 agent 数），量级
  ...
  协议号撞车已在合并时兑现过一次：本条分支开出时占的是 13→14，#1102/#1103
  先合把 main 推进到 15，本条按纪律进位到 16（决策一见协议文件头注）。
