...
- `mobile/tsconfig.json` 给 `@supabase/supabase-js` 加一条 `paths`，指向**手机自己那份**：mobile/ 与仓库根各装了一份，`SupabaseClient` 带 protected 成员，TS 按「是不是同一处声明」判兼容，把手机建的客户端递给 shared 的函数就报 TS2345。指手机那份的理由是**真正跑的客户端实例在 `mobile/src/supabase.ts` 里建出**、shared 调的是它的方法；这条不只管类型：`expo start` 起的 Metro 默认也读 tsconfig 的 `paths`（`app.json` 的 `experiments.tsconfigPaths` 缺省为开，只对 node_modules 之外发起的 import 生效），所以 src/shared/ 哪天在运行时 import 这个包，打进手机包的也是手机那份——类型与打包对的是同一份。关掉那个开关、或者换一个不读 tsconfig 的打包器，这条要重新判。这一段前后写错过两次：提交 9e3a9fee 说它跟着 `nodeModulesPaths` 的顺序走，899f7ad4 又说 Metro 不读 `paths`、shared 会打进根那份——两句都作废，以配置文件里的注释为准。
