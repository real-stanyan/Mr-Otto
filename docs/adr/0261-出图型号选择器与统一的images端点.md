# ADR-0261：模型选单的「文字 / 图像」开关 —— 出图目录扩到七款，整条出图路换到网关的 `/images`

- 原为 ADR-0260（撞号，ADR-0074：本条与 #1091 那篇同时开号，后合的这一篇改成 max+1）
- 状态：已采纳
- 日期：2026-09-08
...
3. **勾落在会被用到的那一款**（`pickImageModel` 的结果），不是落在 `imageModel` 上：
   选过但网关下架了它时，勾也该落在真会跑的那款上。（「没选过」那一档后来归了
   Auto，见决策七。）
4. **浮层每次打开回到「文字」那一格**：触发器上写的是文字模型，开出来停在图像那一格
   就是按钮说一件事、浮层说另一件事。代价是连着改两次出图型号要各点一下「图像」。

名字走 `IMAGE_MODEL_LABELS` 这张**人手维护、会过时**的表（同 `mcpCatalog.ts` 的立场），
认不出的**原样显示 id** —— 网关上了新款而这张表还没跟上时，裸 id 至少是真的。

### 六、厂商标：撤掉「不画」那条（维护者对着真机点的）

初稿写的是「图像那一格不画厂商标」，理由是 `model_route.platform` 对七行出图路由全是
`openrouter`（那说的是上游是谁，不是牌子是谁）。**那个理由只否掉了「从 platform 推」
这一条路，没有否掉「另记一张牌子表」** —— 维护者看过真机后要求加上，这条改回来。

`ImageVendor` 与 `ProviderId` **是两套东西，不合并**：ByteDance 压根不是本仓的一家
模型提供商（没有 key、没有端点、目录里一款聊天模型都没有），往 `providerCatalog` 里
塞一个只为画图标而存在的条目，是让那张表开始撒谎。所以牌子表
（`IMAGE_MODEL_VENDORS`）住在 `shared/imageModel.ts`，与名字表**键逐个对上**，
有断言钉着 —— 只补一张的样子是「有名字没有标」，而那只是少一格、不报错。

**认不出的仍然不画**，也不按 id 前缀猜：网关上一款陌生前缀的新模型时，猜错一家比不画
更坏（ADR-0254 那条原样成立）。方块本体从 `ProviderMark` 里拆成共用的 `Mark`——两格
并排在同一枚选单里，圆角 / 内描边 / 图占比漂移一处就看得出来。

### 七、图像那一格的 Auto，以及它顺手还掉的那笔账

**与文字那格不是同一套机制**：文字那边每轮起跑前真打一次分类调用判难度
（ADR-0237），图像这边只是「由系统挑」—— 今天的规则就是网关清单里最便宜那款。
所以两处的说明文案分开写，没有共用一句「Auto」了事。

真正的收获在另一头：**「没选过」与「显式选了 Auto」合成同一档**（`isImageAuto`）。
分成两格的话，一个从没碰过这一格的人会看到「一行都没勾」，而他其实正在那一档里；
合成之后，勾在 Auto 上还是在某一款上，恰好回答了「这是我选的，还是默认就是它」——
**那正是本 ADR 初稿「代价」第 4 条，这一下还掉了**。

Auto 那个口令（复用 `AUTO_MODEL`）在路由那侧**天然回落最便宜那款**（它不在
`imageModels` 里），`pickImageModel` 一条分支都不用加。不到两款不画 Auto
（同文字那格的 `AUTO_MIN_MODELS`）：那时它与唯一那款是同一件事。

### 八、新会话卡也接上（初稿「代价」第 1 条，同样还掉了）

初稿把它记成已知代价，理由是「还没有会话可落事件」。但欢迎页那张卡上的选择器正是
很多人第一眼看到的那一枚 —— 一个只在会话里才有的开关，等于让人第一次打开就看不到它。
`StartSessionOptions` 加一格 `imageModel`，落地那一刻由主进程 append 事件（同 `model`
走既有通道的立场）；Auto 就是不传，不落一条什么都没改变的事件。

## 代价与已知未做

1. ~~新会话卡那一处不给这一格~~ —— 已在决策八接上。
...
4. ~~勾分不出「我选的」和「默认就是它」~~ —— 已在决策七由 Auto 那一档还掉。
   新的代价是**没选过时看不出 Auto 会挑哪一款**（那句话在 `title` 里），
   与文字那格的 Auto 同构。
5. **图像的 Auto 不判难度**：它今天等价于「最便宜那款」。要做成文字那样的按难度选，
   得在 `generate_image` 之前加一次分类调用，而「这张图难不难」不像文字那样有客观
   判据（贵 4 倍的那款不一定更合意）—— 另开一条再做。
6. 云会话那侧不挂这枚开关（`generate_image` 本来就只在装配了托管的桌面会话上）。
7. 七款目录里 OpenRouter 还有 30 多款没上（Flux / Recraft / Qwen Image / MAI…），
