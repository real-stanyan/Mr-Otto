# ADR-0092: Files 面板是右侧的一个互斥视图，全显树 + 懒加载，严格只读

原为 ADR-0091（与 iOS 模拟器面板那条 lane 撞号，按 ADR-0074 合并前改号）
...
1. **占右侧槽位，与那里的其它视图互斥**（Protocol / Git Graph / 终端 / 浏览器 / DM / iOS 模拟器）。不新开一列：右侧只有一块地方，两块并排会把会话区挤没，而会话才是主体。代价是 Files 和终端不能同屏——接受，两者都是旁路工具，同时要看的场景稀少。
