...
8. 没开麦时（静音、断线、第一次等授权）每放完一段都停引擎、交还会话，下一段再起：段与段之间听得见一个缺口（放音队列的预取补不上它），每段还多一次会话的起落。整场通话引擎与会话都不停、静音只摘 tap，另开一条。
