# ADR-0121：MCP 的 OAuth 授权——回调走 loopback 临时端口，凭据存独立文件

> 原为 ADR-0112，后改 0117，现为 0121（三次撞号：0112 被先合的 PR 占走；改 0117 后本车道合并时没 re-fetch，与已在 main 上的 ADR-0117「一个厂商条目 = 一个端点 + 一把 key」撞了个正着，撞号顶着红门禁进了 main。按项目 ADR-0074 的 claim-at-merge，每次取当时最大号 +1）。照旧号来的引用按这行解析。
