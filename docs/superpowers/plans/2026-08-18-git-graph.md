...
describe("assignLanes", () => {
  ...
  it("第二父已有道在等:直接汇入既有道,不开新道", () => {
    ...
    // m 落 0,b 落 1;m 的 merge 线并入 1 道(fromLane 0 → toLane 1)
    expect(rows[0].lane).toBe(0);
    expect(rows[1].lane).toBe(1);
    expect(rows[3].lane).toBe(1); // shared 落在等它的 1 道
  });
...
describe("commit", () => {
  it("组合 show --no-patch 元数据 + numstat", async () => {
    const svc = createGitGraphService(fake({
      onExec: async (args) => {
        if (args.includes("--no-patch")) return { stdout: `aaa\x00stan\x00s@x.com\x001755500000\x00feat: x\n\n正文\n` };
        return { stdout: "3\t1\tsrc/a.ts\n" };
      },
    }));
    const r = await svc.commit("/repo", "aaa");
    expect(r).toEqual({
      ok: true,
      detail: {
        hash: "aaa", author: "stan", email: "s@x.com", timestamp: 1755500000,
        body: "feat: x\n\n正文",
