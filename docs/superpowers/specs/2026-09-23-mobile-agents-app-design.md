...
79. **开电话的那一拍那一格已经是 live**：runtime 先广播名单再回执，名单落下来时这台还没开始听，照「没在听」画会先闪一下「通话还开着」；只认开电话这一个动作（从「通话还开着」那一格挂断不会把它翻成 live），那一拍麦算开着。同一处判据（`callMicOn`）：**没权限时麦算关着**（声浪一条灰线、那颗钮写「取消静音」）——没权限那句话叫人点一下麦克风，算开着的话一点就是关麦、把那句话抹掉；识别出错仍算开着（原生自己在重试）。
