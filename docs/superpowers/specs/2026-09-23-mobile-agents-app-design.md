...
### 5.7 语音（A4）
电话模式**替换输入框**（脸 m 档在说 / 在听、声浪、计时、转文字 / 静音 / 挂断）；挂断折成一张卡（「多久 + 聊的什么」，同 ADR-0288），点开是从下面滑上来的一扇窗、不盖住头上那颗药丸。依赖：`call` 帧与 `voice_call_changed`（已有）、TTS 走 edge `/llm/v1/speech` + `expo-audio` 放音、识别要 dev build（Expo Go 不带 STT）、音色可选 = `workspace_agents` 加一列 + ADR（demo 六档对到 12 个 MiniMax 音色里挑哪六个，届时定）。实现以 §10 第 57–79 条为准（plan：`docs/superpowers/plans/2026-09-26-mobile-agents-a4-voice.md`，ADR-0320）。
...
60. **声音按 agent_id 派生**（桌面同一份 `agentVoiceId`），「说话的声音」那一格与挑声音另开一片（维护者 2026-09-26，#1372）。
...
73. **原生模块的命令、回调、定时器全在它自己的一条串行队列上跑，不在主线程**（桌面 MrOttoSpeech 原样搬过来的是主线程）：iOS 的主线程就是界面线程，而配 AVAudioSession、`setActive`（同步阻塞）、开回声消除、起引擎加起来常有几百毫秒，换路由 / 蓝牙能到几秒——放在主线程上，开电话那一刻、没开麦时每放一段，界面都要停一下。桌面那份没事，是因为那是 helper 自己进程的主线程。串行保住 start / stop / play 的先后（ADR-0320 决定 1）。
74. **开麦带一个号**：开麦要等两道授权（第一次会弹窗），等的时候来了停麦（切后台 / 离开 / 挂断 / 房间被拒），授权回来时这次开麦作废——JS 那边早当它关了、不会再发一次停，不作废的话麦就开着没人管（橙点亮着、识别在跑）。作废的这次不报「麦关了」：它没开过。桌面 helper 有同一个形状，另开。
75. **「声音设备变了」按引擎此刻停没停判，不按时间窗**：系统是先停引擎再发这条通知的；我们自己在起引擎之前开回声消除也会引出一条，它排到那条队列上时引擎已经起来了——还在跑就是那一条。原来的「起来之后 1 秒内的不算」会把起来之后真停掉的那一次也吞掉（显示在听、什么都识别不到、一个字不说）。不自动重起（维护者裁定），要人再点一下麦克风。回声消除开不成时留一行系统日志，界面上不说（退回半双工不是错）。
76. **半双工在开麦与 status 上重新对齐**（shared 的 `voiceSession`）：原生报「麦开了」时按此刻该不该闭麦重新对一遍（原生开麦时是没闭着的，开麦前发的闭麦命令它丢掉了），每条 status 也对一遍（回声消除开没开可能变了）；回声消除开没开跨过重新开麦记着，不退回「不知道」。不对齐的话，没有回声消除时它自己的话会被录进麦克风、当成人说的发出去（#1176 的自激）。桌面 store 有同一个缺口，改用 `voiceSession` 时一起补上。
77. **放一段起不来（音频解不开 / 引擎起不来）时那条 play 命令被拒，不发 playError**：命令还没回时发出去的事件可能赶在 JS 登记那一段之前到，那一段就永远等不到回执（放音队列卡住，半双工时麦一直闭着）。被拒的 message 就是原话（界面上是「播放失败：音频解不开：…」），不带 expo 包的那一层类名与源文件行号。起来之后的结局（放完 / 被打断）照旧走事件。
78. **「接着听」那一格按房间状态说**（第 72 条的细化）：在连写「正在连上这条聊天…」、断线写「正在重连这条聊天…」、被拒写「这条聊天连不上了，接不了电话。」（终态，不说「正在」——那会让人一直等）；扣款没成功（past_due）单独一句「这个账号的订阅扣款没成功，续上之后才打得了电话。」——它不是没订阅（ADR-0240），手机上 A5 之前没有账号页，不指去哪儿续。
79. **开电话的那一拍那一格已经是 live**：runtime 先广播名单再回执，名单落下来时这台还没开始听，照「没在听」画会先闪一下「通话还开着」；只认开电话这一个动作（从「通话还开着」那一格挂断不会把它翻成 live），那一拍麦算开着。同一处判据（`callMicOn`）：**没权限时麦算关着**（声浪一条灰线、那颗钮写「取消静音」）——没权限那句话叫人点一下麦克风，算开着的话一点就是关麦、把那句话抹掉；识别出错仍算开着（识别中断那一族原生自己在重试）。**已知缺口**：开麦本身失败时原生只报 error、不补 listening 关，那颗钮因此仍写「静音」（#1369）。
...
7. **「说话的声音」可选另开一片**：A4 照桌面按 agent_id 派生，不加列（2026-09-26，#1372）。
