...
> 2026-08-26 订正：本句原本还列了「授权服务器元数据缓存」一项，那是写在「以为要自己实现 OAuth 协议」的阶段。§4 后来订正为「不自己实现 OAuth 协议」——元数据发现归 SDK，`OAuthClientProvider` 上的 `saveDiscoveryState` / `discoveryState` 是可选成员，本仓不实现，元数据每次连接由 SDK 自己重新发现、不落盘。这句字段清单当时漏改，实现（`McpAuthRecord`）始终只有上面三项。详见 ADR-0121。
