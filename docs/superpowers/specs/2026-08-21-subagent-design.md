...
> 订正（2026-08-23，issue #141）：**派活的那条 `task` 工具行压掉，只留卡。**
> 原来一次派活在时间线上渲染两遍——一张派活卡，外加一条普通工具行（原始参数 +
> 汇报全文）。卡上已经有 agent、任务、状态、汇报和"点进子会话"的入口，工具行
> 是同一份信息的原始形态，对读的人没有新东西。判据是"这条调用真的落过
> `subagent_spawned`"，不是"工具名叫 task"：派活之前就失败的那次（没有这个
> 名字的 subagent / 还没派出去就被中断）根本没有卡，按工具名一律压会把唯一的
> 报错也吞掉。实现见 `spawnedToolCallIds`。
