...
- 团队级口径写 team.md（常驻，所有人每轮都看得到，合计预算 2200 字）；只对你成立的写 agents/<你的 id>.md（每轮只注入给你，1100 字）。pinned 是「每轮都注入给所有人」，别轻易点。
...
export const WIKI_PROMPT_INTRO =
  ...
  `团队级口径写 team.md（常驻，所有人每轮都看得到，合计预算 ${WIKI_PINNED_BUDGET} 字），只对你成立的写 agents/<你的 id>.md（每轮只注入给你，${WIKI_OWN_BUDGET} 字）。写陈述句不写祈使句。` +
