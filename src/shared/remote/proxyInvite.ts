...
/** 编码成可带外传输的字符串（DM/当面发）。五段冒号分隔，公钥/secret 走 base64 */
...
export function decodeProxyInvite(text: string): ScannedProxyInvite | null {
  ...
  const [prefix, vStr, channelId, pubB64, secretB64, tsStr] = parts as [
  ...
  if (!channelId || !Number.isFinite(createdTs)) return null;
  return { v: PROXY_FRAME_VERSION, channelId, hostIdentityPub, secret, createdTs };
}
