...
export interface CloudSessionStatus {
  ...
  /** 这条会话是哪一种聊天、名单是谁（#1280）。**三态**（#1301）：**缺席 = welcome
      还没到**（还不知道）/ `null` = welcome 到了、说这是团队会话 / 值 = 一条聊天。

      原来缺席同时表达「团队会话」与「还没问到」，渲染层照前者画——于是进一条聊天
      的头几秒（真机十秒以上）主区是团队壳，主场里还露出一颗点了不生效的「免审批」
      开关。welcome 每条连接只来一次，所以「缺席」在这里是个**会自己结束**的状态，
