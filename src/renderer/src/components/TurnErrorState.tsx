...
//
// 本仓要的两处语义,element 的 props 都留了口(见 error-state.tsx 里的"本仓改动"):
// ① 出口可能不存在(历史里的旧失败行 / turn 正在跑)—— 没有出口就不长钮
// ② retrying 那一档只属于"当下这一条":历史行不该因为现在有个 turn 在跑就变成转圈
// 重试只有一档了:原样重发,附件一起(ADR-0042)
...
export function TurnErrorState({
  ...
  const canRetry = interactive && !retrying && prev !== null;
  ...
  return (
    <ErrorState
      ...
      {...(canRetry
        ? {
            onRetry: () => retryLastUserMessage(prev!),
            retryLabel: "重试",
            retryTitle: "重试：把上一条消息原样再发一遍（附件一起）",
          }
        : {})}
