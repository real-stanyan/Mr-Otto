...
import { field, ghostButton, mono, paper } from "@/lib/surfaces.js";
...
> & {
  ...
  queuedLabel?: (count: number) => ReactNode;
  hint?: ReactNode;
}) {
  return (
    <div
      ...
      {queued.length > 0 && (
        <div className="flex items-baseline justify-between px-1">
          <span className={cn(mono, "text-foreground/35")}>
            {queuedLabel ? queuedLabel(queued.length) : `${queued.length} queued`}
          </span>
          <span className={cn(mono, "text-foreground/35")}>
            {hint ?? "sends when this finishes"}
          </span>
        </div>
      )}
        ...
        {queued.map((message, index) => (
          ...
          >
            ...
            <button
              type="button"
              // 本仓改动:中文界面
              aria-label={`把「${message.text}」移出队列`}
