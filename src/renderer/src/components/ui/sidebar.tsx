...
/* shadcn 原版的移动端分支(useIsMobile 768px 断点 → 侧栏换成 Sheet)已整个拆掉：
   ...
   而唯一的 SidebarTrigger 就在 Sheet 内部跟着一起藏了，同时 CollapsedSidebarNub
   判的 state 只跟桌面态的 open 走(仍是 expanded)也不渲染——界面上一颗能打开侧栏的
   按钮都不剩，只有 ⌘B。桌面态的 offcanvas 收起本来就够用，不需要第二套形态。 */
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  ...
  return (
    <button
      ...
      className={cn(
        "absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 transition-all ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] hover:after:bg-sidebar-border flex",
