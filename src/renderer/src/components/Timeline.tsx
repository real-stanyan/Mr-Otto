...
import { GitBranch, ImageIcon, KeyRound, Mail, Share2 } from "lucide-react";
import type {
...
export const EventRow = memo(function EventRow({ event, isLast = false }: { event: SessionEvent; isLast?: boolean }) {
  ...
  switch (event.type) {
    ...
    // 这回事。用 AUDIT 那种缩进小字，量纲与「请求信封已更新」同一档。
    // 落成一行而不是完全不画：这一格决定下一张图花多少钱（七款差 4 倍），
    // 往回翻时「这张图是谁画的」只有这一行答得出
