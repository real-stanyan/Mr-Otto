...
import { CtxBreakdownSection, CtxCard } from "@/components/CtxBreakdownSection.js";
...
/** 用量环的详情浮层：全部数字都是投影（日志 + 主进程报的工具表），没有独立状态。
    主视觉 = 一条按来源分段的占用条 + 图例，回答"上下文被谁吃掉了"——那一段（hero /
    条 / 图例）连同壳（CtxCard）住在 components/CtxBreakdownSection.tsx，云会话那张卡
    （CloudContextRing）与这里共用（#1138）；这里只剩本地会话独有的上下两段：
    套餐额度、钱或脚注。
...
function CtxDetails({ events, toolDefs, ctxWindow }: {
  ...
  return (
    <CtxCard>
      ...
      {/* 会话那半 */}
      <CtxBreakdownSection breakdown={breakdown} ctxWindow={ctxWindow} />
...
function AppearancePage() {
  ...
  // 动效(#607)。null = 还没读回来(同 AutoCompactSettings 的 loaded 模式)。
...
export function App() {
  ...
  // 原来还有一份 sectionAnchors(分区轨的锚点):分区轨换成了会话地图(ADR-0292),
  // 条目、量位置、跳转都住进了 OttoThread,App 这边一行接线都不剩
  ...
  const base = settingsSection === "account" ? (
    ...
    <div className={MAIN_COL}>
      ...
      {replaying ? (
        ...
        <OttoRuntimeProvider>
          {/* relative 不是随手写的:会话地图(OttoThread 里的 ConversationMapRail,ADR-0292)
              是挂在这一层上的绝对定位浮层 —— 出现和消失都不动布局。这层同时是划词引用
              (SelectionQuote)的宿主 */}
