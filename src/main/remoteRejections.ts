 ...
 * 而不是在配对成功时去清一个标志位。
 *
 * 为什么不清标志位:被拒的那台**在用户去配它之前会一直被拒**,而配上的那台每次
 * 连上来都成功。"握手成功就清提示"于是会让成功的那台不断把被拒那台的提示抹掉 ——
 * 而后者恰恰是用户唯一需要看见的。按 deviceId 现算就没有这个问题:
 * 配上了自然不显示,没配上一直显示。
 *
 * (这段原来的写法是"一台桌面只 pin 得住一把公钥,所以第二台是永久被拒的"。
 * 多设备配对之后前半句不再成立,但结论没变 —— 变的只是"永久"现在指的是
 * "在用户去配它之前"。)
 */
