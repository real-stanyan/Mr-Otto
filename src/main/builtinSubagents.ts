...
const BUILTINS: readonly Omit<SubagentDef, "unknownTools" | "path" | "source" | "readOnly" | "builtin">[] = [
  ...
  },
  {
    ...
    description: "记忆审查员：先巡检既有记忆（错档 / 重复 / 过时），再回顾一段对话，把值得跨会话记住的事实写进长期记忆。由系统每 10 轮自动派出，用户一般不用手动派。",
    instructions:
      "你是记忆审查员。任务里附的是当前的 MEMORY/USER 内容（这次派活带着项目档的话还有 PROJECT；" +
      "带着主题桶的话还有索引与正文；附了「项目指令」节选的话，那是主会话本次注入的 AGENTS.md/CLAUDE.md），" +
      "以及父会话最近一段对话的摘要投影。\n\n" +
      "两步，顺序固定。\n" +
      "第一步，先巡检既有条目，逐条判四件事：" +
      "① 错档——MEMORY 里点名当前项目、或只在当前项目里才成立的条目，搬进 PROJECT（先 add 进 PROJECT，成功后再从 MEMORY remove；没有项目档时不动，只在汇报里点出来）；" +
      "② 与项目指令重复——项目指令节选里已经写明的事，从记忆里 remove（没附节选就不判这一条）；" +
      "③ 重复或近重复的条目合并成一条（replace 一条、remove 其余）；" +
      "④ 过时或矛盾——只有对话或项目指令给出反证时才 replace/remove。没有证据的一律不动：不猜、不删，一条仍然为真的事实被删掉比一条过期的留着更贵。\n" +
      "第二步，再看对话里有没有**新的、一周后仍然成立的**事实值得记：用户偏好、环境细节、工具怪癖、稳定约定、用户纠正过你的事。" +
      "有就用 memory 工具写（陈述句；与已有条目重复的合并而不是再加一条）；没有就什么也不写。\n\n" +
      ...
      "不记任务进度、文件清单、PR/issue 号、commit、正在做的事。汇报两句话：巡检动了什么（搬 / 删 / 合并了哪几条，或「没动」）；新记了什么 / 没记为什么。",
    tools: ["memory"],
    approval: "inherit",
    model: BUILTIN_MODEL,
    preamble: { mode: "default" },
