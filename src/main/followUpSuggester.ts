// followUpSuggester — 跟进建议的生成员。住在 engine 外面的"外挂"(engine 不认识它):
// 位置在 turn 收口之后、turn 锁之外,纪律是永不抛、失败静默、不落事件而已。
//
// 建议不需要自愈 —— 它是"此刻这一屏"的东西,这个 turn 没生成出来,下个 turn
// 自然会有新的一批。所以这里没有锚点那一套,只看最后一轮问答。
//
// 模型调用本体在 turnAnnotator.ts（issue #284:与自动命名、主题分类合并成一次往返）;
// 这里只剩判定逻辑——取最后一轮、摘要、解析清洗都是纯函数。

import type { SessionEvent } from "../session/events.js";

/** 要几条。三条是有意的上限:建议是快捷键不是菜单,多到要读就已经比自己打字慢了。
    导出给 turnAnnotator 的提示词用——解析封顶和提示词要的条数必须是同一个数 */
export const WANT = 3;
