...
export interface UserMessageEvent extends SessionEventBase {
  ...
  /** 云会话群聊（#928 / #932）：这句话是哪个成员说的。**只在 runtime 落的
      ...
      展示名比对（那是 1a 的将就：同名两个人就分不开）。**模型投影只把它当
      判据用**（issue #965）：在场 = 这条正文已经/将要拼进 `[label]: text`
      这类结构，过 `promptSafeBody` 拆穿正文里的伪造说话人行；缺席（本机
      操作者/旧日志）则一个字节不动——它本身的字符串值不进模型上下文 */
  fromUid?: string;
  ...
  /** 这句话是**在语音通话里说出来的**（#1233）。缺席 = 打字打的 / 旧日志。
      **只是记号**：起 turn、排队、护栏、接力链首、派活全都不看它，模型投影
      ...
      逐字节一致（同 `origin` / `dispatch` 那两格的纪律）。

      它存在的唯一理由是**云会话时间线要把一场通话折成一张卡**（ADR-0288）：
      通话里说的话原来一句一条气泡，一场 12 句的通话把时间线撑开一千多像素。
      而「哪几条属于这场通话」在日志里此前判不出来——`voice_call_changed` 只说
      名单变了，转写出来的这条与手打的那条一个字节都不差。
      **否决了「按 seq 区间吞」**：那条路零 schema 改动，但云会话是群聊，通话
      期间**没在通话里的人打的字会被吞进一张他没参与的通话卡**——而这条 issue
      修的正是「不该进时间线的东西进了时间线」，方向修反就是同一类 bug 的新一版。
      第三条路（按「谁在通话里」过滤人类发言）判不出来：人类「谁在听」不落盘
      （ADR-0271 的已知代价）。
...
export interface ChatMessageEvent extends SessionEventBase {
  ...
  /** 这句话是在语音通话里说出来的（#1233）。语义与落点逐字同
      ...
      chat_message。通话里 ADR-0275 保证「必须有人应」，所以正常情形走前者；
      走后者的是三种真实的边角——被限速（`budget` 否决了派活）、名单降级、
      以及通话刚结束而麦克风最后一句 final 才到。只给一个字段 = 那三种情形下
      这句话从卡里漏出来，独自躺在时间线上。 */
  voice?: true;
}
