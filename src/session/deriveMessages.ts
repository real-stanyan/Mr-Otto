...
/** 云会话（工作区群聊）的回复口径（#989 → #1132，ADR-0266）：群里的读者是
    ...
    加粗编号小标题 + 一长串「- 」+ 四五百字一口气说完（维护者原话「缺乏活人感，
    ...
    ④ 气泡是纯文字，Markdown 记号会原样露出来——对「**3. 小标题**」那种星号
       满屏。
    每一条都说得出**替代写法**，只说「别用」模型会退回它最熟的那套。真正的代码
    （别人要它写的脚本、命令）照常用代码围栏，那不是回复格式，是交付物。
    它跟着每一次云请求走，所以刻意压在两百来字 */
const PLAIN_TALK =
  `\n群里的人来自各行各业，不都是开发者。像同事在群里聊天那样说话：口语、短句、先说结论和下一步；` +
  `别甩术语，非用不可就顺手用一句白话解释。\n` +
  `别一次把细节全倒出来——两三段说清主线就停，对方想细看再展开。段与段之间空一行、一段只说一件事：` +
  `群里会把每一段当成你连发的一条消息。\n` +
...
export function renderVoiceCallPrompt(
  ...
  return (
    ...
    // 名单以这一块为准（#1194）：上一场通话的招呼还留在日志里，模型会拿它们当这一场的名单
    // （真机：只有管理员在，它说「开发刚才进来打过招呼，设计也在」）
