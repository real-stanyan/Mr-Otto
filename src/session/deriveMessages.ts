...
/** 云会话（工作区群聊）的回复口径（#989）：群里的读者是各行各业的人，不是
    开发者。STRUCTURED_BLOCKS 那五种围栏在云时间线上不渲染成卡片，写出来就是
    一块 JSON 代码格子；表格、JSON、术语同理。要列东西就短句分行——这一条要
    说得出**替代写法**，只说「别用」模型会退回它最熟的表格。真正的代码
    （用户要它写的脚本、命令）照常用代码围栏，那不是回复格式，是交付物 */
const PLAIN_TALK =
  `\n群里的人来自各行各业，不都是开发者：回复用平常说话的方式，别用 JSON、表格、` +
  `\`\`\`otto-xxx 这类格式当回复的骨架，别甩术语（要用就顺手解释一句）；要列几件事就一行一件短句说。` +
  `只有用户真要的代码、命令才放进代码围栏——那是交付物，不是说话的方式。\n`;
